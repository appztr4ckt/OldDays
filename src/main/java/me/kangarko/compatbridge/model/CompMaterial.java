...
public enum CompMaterial {
   ...
   private CompMaterial(String legacyName, int data) {
      ...
      try {
         this.material = Material.valueOf(VersionResolver.isAtLeast1_13() ? this.name() : parseLegacyName(legacyName));
   ...
   public final ItemStack toItem(int amount) {
      ...
      return VersionResolver.MC_1_13 ? new ItemStack(mat, amount) : new ItemStack(mat, amount, (byte)this.data);
   }

   public final Material toMaterial() {
      Material mat = Material.matchMaterial(this.toString());
      return mat != null ? mat : Material.matchMaterial(this.legacyName);
   }
   ...
   public final boolean is(ItemStack comp) {
      if (VersionResolver.MC_1_13) {
         return comp.getType() == this.toMaterial();
      } else if (comp.getType() == this.toMaterial() && comp.getData().getData() == this.data) {
         return true;
      } else {
         CompMaterial xmat = fromMaterial(comp.getType());
         return xmat != null && isDamageable(xmat) && this.toMaterial() == comp.getType();
      }
   }
   ...
   public static final boolean isDamageable(CompMaterial type) {
      if (type == null) {
         ...
         String var1;
         switch((var1 = type.toString()).hashCode()) {
         case -1850133582:
   ...
   public static ItemStack makeWool(CompDye color, int amount) {
      if (VersionResolver.MC_1_13) {
         ...
         ItemStack wool = new ItemStack(Material.valueOf("WOOL"), amount);
   ...
   public static Material makeMonsterEgg(EntityType type) {
      ...
      if (VersionResolver.isAtLeast1_13() && type == EntityType.PIG_ZOMBIE) {
   ...
   public static Material getCarpetMaterial() {
      return getCarpetMaterial(CompDye.WHITE);
   }

   public static Material getCarpetMaterial(CompDye color) {
      return fromLegacy("CARPET", color.getDye().getDyeData()).getMaterial();
   }

   public static Material getWoolMaterial() {
      return getWoolMaterial(CompDye.WHITE);
   }

   public static Material getWoolMaterial(CompDye color) {
      return fromLegacy("WOOL", color.getDye().getDyeData()).getMaterial();
   }
   ...
   public static CompMaterial fromLegacy(String name, int data) {
      ...
      if (cachedSearch.containsKey(name + "," + data)) {
         ...
         try {
            mat = valueOf(name);
            return fromLegacy(mat.legacyName, data);
         } catch (IllegalArgumentException var6) {
            return null;
         }
      }
